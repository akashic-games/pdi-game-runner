...
export interface DynamicAMFlowImplParams {
...
export class DynamicAMFlowImpl extends StaticAMFlowImpl {
	...
	constructor(dynamicAMFlowImplParams: DynamicAMFlowImplParams) {
		super(dynamicAMFlowImplParams.config, dynamicAMFlowImplParams.permission, dynamicAMFlowImplParams.playlogStore);
		this._config = dynamicAMFlowImplParams.config;
		this._tickExchange = null;
		this._eventExchange = null;
		this._eventQueue = null;
		this._tickHandlers = [];
		this._eventHandlers = [];
		this._eventFilters = dynamicAMFlowImplParams.eventFilters || [];
		...
		this._tickExporter = dynamicAMFlowImplParams.tickExporter;
	}
	...
	removeTransientEvent(tick: playlog.Tick): playlog.Tick {
		...
		replacedTick[pidx.Tick.Storage] = (tick[pidx.Tick.Storage] && tick[pidx.Tick.Storage].length)
			? tick[pidx.Tick.Storage]
			: undefined;
		return replacedTick;
	}
	...
	protected _publishAmqp(exchange: string, data: Uint8Array, callback: (err: Error) => void): void {
