...
export class DynamicAMFlowImpl extends StaticAMFlowImpl {
		...
		chWrapper.open()
			.then(() => {
				if (!this._tickExporter) {
					return this._declareTickExchange(chWrapper);
				}
			})
			.then(() => {
				if (!this._tickExporter) {
			...
			.catch(err => {
				...
				if (callback) {
					runnerProcess.setImmediate(() => { callback(AMFlowError.createRuntimeError("open failed", err)); });
				}
			});
	}
	...
	sendTick(tick: playlog.Tick): void {
		...
		if ((tick[pidx.Tick.Events] && tick[pidx.Tick.Events].length)
			...
			data = msgpack.encode(tick);
		} else {
			data = msgpack.encode(tick[pidx.Tick.Frame]);
		}
		this._publishAmqp(this._tickExchange, data, (err) => {
			if (err) throw AMFlowError.createRuntimeError("send tick failed.", err);
		});
		this._playlogStore.storeTick(tick[pidx.Tick.Frame], data);
	}
