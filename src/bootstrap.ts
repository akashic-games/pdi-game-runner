...
import { GameExternalStorageTransactionAccessImpl } from "./plugins/contentStorage";
...
// setup uri whitelist
runnerProcess.setupAccessibleUriList(config.accessibleUriList);
...
// externals
if (akashicEngineParametersMod.externals) {
	akashicEngineParametersMod.externals.forEach(ext => {
		if (ext.code === mods.AKASHIC_ENGINE_EXTERNAL_CONTENT_STORAGE_CODE) {
			if (!runnerProcess.contentStorageRedisEnabled) {
				throw new Error("content storage redis is not enabled.")
			}
			externals[mods.AKASHIC_ENGINE_EXTERNAL_CONTENT_STORAGE_CODE] = new GameExternalStorageTransactionAccessImpl();
		}
	});
}
	...
	runnerProcess.openAmqpConnection((event, msg) => {
		if (event === "connected") {
			...
			if (amflowImpl && gameDriver) {
				...
				gameDriver.stopGame();
				launchTimeout = Date.now() + 30000;  // 10 秒
				launchTimeoutLoop.start();
			}
		}
	});
