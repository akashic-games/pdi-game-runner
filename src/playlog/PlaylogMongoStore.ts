...
export class PlaylogMongoStore extends PlaylogStoreBase {
	...
	_ignorablePostfix: string = "_ignored";
	...
	getKey(ignorable: boolean): string {
		return ignorable ? this._playId + this._ignorablePostfix : this._playId;
	}

	storeTick(frame: number, data: Uint8Array, ignorable?: boolean): void {
		const playId = this.getKey(ignorable);
		this._storeData(this._tickNs, {playId, frame: frame}, "data", data, err => {
	...
	getTickList(begin: number, end: number, callback: (error: Error, tickList: TickList) => void, ignorable?: boolean): void {
		const playId = this.getKey(ignorable);
		const query = {playId, frame: {$gte: begin, $lt: end}};
		const sort = {frame: 1};
		runnerProcess.mongodbGetBinaryField(this._tickNs,
			...
			if (!bins.length) {
				...
				// ignorableかつ結果が空だった場合は通常のplayIdで取得しなおす
				const query = {playId: this._playId, frame: {$gte: begin, $lt: end}};
				runnerProcess.mongodbGetBinaryField(this._tickNs, JSON.stringify(query), JSON.stringify(sort), "data", 0, 0, (err, bins) => {
					if (err) {
						callback(AMFlowError.createRuntimeError("get ticks from store failed", err), null);
						return;
					}
					...
					try {
						const result = this.createTickList(begin, bins);
	...
	putStartPoint(startPoint: StartPoint, callback: (error: Error) => void): void {
		...
		this._storeData(this._startPointNs, {playId: this._playId, frame: startPoint.frame, timestamp: startPoint.timestamp},
